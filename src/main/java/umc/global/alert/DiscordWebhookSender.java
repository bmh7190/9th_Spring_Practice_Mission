...
public class DiscordWebhookSender {
    ...
    public void sendErrorEmbed(ErrorEmbed ctx, int topN) {
        if (!props.enabled() || isBlank(props.webhookUrl())) return;
        ...
        embed.put("footer", Map.of("text", joinNonBlank(ctx.environment(), ctx.appName(), " • ")));

        var payload = new HashMap<String, Object>();
        if (!isBlank(props.username())) payload.put("username", props.username());
    ...
    private String topStack(Throwable t, int n) {
        ...
        for (int i = 0; i < limit; i++) sb.append(st[i]).append('\n');
    ...
    private String sanitize(String s)           { return s == null ? "" : s.replace("```","`\u200B``").replace("@everyone","@ everyone"); }
